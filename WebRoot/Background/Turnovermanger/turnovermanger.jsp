...
$(function(){
	...
	function getData(){
		...
		$.ajax({
			...
			type:"post",
			data:"",
			url:"../TurnovermangerAction!turnoverList",						
			dataType:"json",
...
//表单数据转json数据格式
	...
	var url;
//查询客户
    function searchTurnovermanger() {
        $("#tt").datagrid('options').url="../TurnovermangerAction!searchTurnover";
		$("#tt").datagrid('load',{
			"turnover.id":$("#id").val()
		});
    }
//打开新增客户窗口
	function openTurnovermangerAddDialog() {
		$("input[type=reset]").trigger("click");
		$("#dlg").dialog("open").dialog("setTitle", "添加营业额信息");
		url = "../TurnovermangerAction!addTurnover";
	}
//打开修改客户窗口
	function openTurnovermangerModifyDialog() {
		$("input[type=reset]").trigger("click");
		...
		$("#dlg").dialog("open").dialog("setTitle", "编辑营业额信息");
		$("#fm").form("load", row);
		url = "../TurnovermangerAction!updateTurnover";
	}
//ajax方式提交form表单	
	function saveTurnovermanger() {
		$.ajax({
			type : "POST",
			dataType : "json",
			url : url,
			...
			success : function(data) {
				...
				$("#dlg").dialog("close");
				$("#tt").datagrid({url:"../TurnovermangerAction!turnoverList"});							
				$("input[type=reset]").trigger("click");
			},
			error : function(data) {
...
//关闭窗口
	function closeTurnovermangerDialog() {		
		$("#dlg").dialog("close");	
		$("input[type=reset]").trigger("click");	
	}
//删除用户
	function deleteTurnovermanger() {
		var selectedRows = $("#tt").datagrid("getSelections");
		...
		$.ajax({
			type : "POST",
			dataType : "json",
			url : "../TurnovermangerAction!deleteTurnover",
			data : "ids="+ids,
			success : function(data) {
...
<div id="body" region="center" > 
    ...
    <div id="conditon">
      <table border="0">
        <tr>
          ...
          <td>
              <a  href="javascript:searchTurnovermanger()" class="easyui-linkbutton my-search-button" iconCls="icon-search" plain="true">查询</a> 
  ...
  <!-- 表格顶部工具按钮 -->
    <div id="tb">
        <a href="javascript:openTurnovermangerAddDialog()" id="ddd" class="easyui-linkbutton" iconCls="icon-add" plain="true">新增</a> 
        <a href="javascript:openTurnovermangerModifyDialog()" class="easyui-linkbutton" iconCls="icon-edit" plain="true">修改</a> 
        <a href="javascript:deleteTurnovermanger()" class="easyui-linkbutton" iconCls="icon-remove" plain="true">删除</a>
    </div>
   <!-- 添加客户信息及修改客户信息 弹出框 -->
      ...
      &nbsp;<span style="color: red">*</span></td>
		...
		<div id="dlg-buttons">
            <a href="javascript:saveTurnovermanger()" class="easyui-linkbutton" iconCls="icon-ok">保存</a> 
            <a href="javascript:closeTurnovermangerDialog()" class="easyui-linkbutton" iconCls="icon-cancel">关闭</a>
        </div>
</div>
</body>
</html>