...
$(function(){
	...
	function getData(){
		...
		$.ajax({
			...
			type:"post",
			data:"",
			url:"../RepairnummangerAction!repairnummangerList",						
			dataType:"json",
...
//表单数据转json数据格式
	...
	var url;
//查询客户
    function searchRepairnum() {
        $("#tt").datagrid('options').url="../RepairnummangerAction!searchRepairnum";
		$("#tt").datagrid('load',{
			"repairnum.id":$("#id").val()
		});
    }
//打开新增客户窗口
	function openRepairnummangerAddDialog() {
		$("input[type=reset]").trigger("click");
		$("#dlg").dialog("open").dialog("setTitle", "添加维修信息");
		url = "../RepairnummangerAction!addRepairnummanger";
	}
//打开修改客户窗口
	function openRepairnummangerModifyDialog() {
		$("input[type=reset]").trigger("click");
		...
		$("#fm").form("load", row);
		url = "../RepairnummangerAction!updateRepairnummanger";
	}
//ajax方式提交form表单	
	function saveRepairnummanger() {
		$.ajax({
			type : "POST",
			dataType : "json",
			url : url,
			...
			success : function(data) {
				...
				$("#dlg").dialog("close");
				$("#tt").datagrid({url:"../RepairnummangerAction!repairnummangerList"});							
				$("input[type=reset]").trigger("click");
			},
			error : function(data) {
...
//关闭窗口
	function closeRepairnummangerDialog() {		
		$("#dlg").dialog("close");	
		$("input[type=reset]").trigger("click");	
	}
//删除用户
	//删除用户
	function deleteRepairnummanger() {
		var selectedRows = $("#tt").datagrid("getSelections");
		...
		$.ajax({
			type : "POST",
			dataType : "json",
			url : "../RepairnummangerAction!deleteRepairnumamnger",
			data : "ids="+ids,
			success : function(data) {
...
<div id="body" region="center" > 
    ...
    <div id="conditon">
      <table border="0">
        <tr>
          ...
          <td>
              <a  href="javascript:searchRepairnum()" class="easyui-linkbutton my-search-button" iconCls="icon-search" plain="true">查询</a> 
  ...
  <!-- 表格顶部工具按钮 -->
    <div id="tb">
        <a href="javascript:openRepairnummangerAddDialog()" id="ddd" class="easyui-linkbutton" iconCls="icon-add" plain="true">新增</a> 
        <a href="javascript:openRepairnummangerModifyDialog()" class="easyui-linkbutton" iconCls="icon-edit" plain="true">修改</a> 
        <a href="javascript:deleteRepairnummanger()" class="easyui-linkbutton" iconCls="icon-remove" plain="true">删除</a>
    </div>
   <!-- 添加客户信息及修改客户信息 弹出框 -->
      ...
      &nbsp;<span style="color: red">*</span></td>
		...
		<div id="dlg-buttons">
            <a href="javascript:saveRepairnummanger()" class="easyui-linkbutton" iconCls="icon-ok">保存</a> 
            <a href="javascript:closeRepairnummangerDialog()" class="easyui-linkbutton" iconCls="icon-cancel">关闭</a>
        </div>
</div>
</body>
</html>