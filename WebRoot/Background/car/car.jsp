...
//查询客户
	function searchCar() {
		$("#tt").datagrid('options').url="../CarAction!searchCar";
		$("#tt").datagrid('load',{
			carNo:$("#carNo").val()
		});
	}
//打开新增客户窗口
	function openCarAddDialog() {
		...
		$("#dlg").dialog("open").dialog("setTitle", "添加车型信息");
		url = "../CarAction!addCar";
	}
//打开修改客户窗口
	function openCarModifyDialog() {
		...
		$("#dlg").dialog("open").dialog("setTitle", "编辑车型信息");
		$("#fm").form("load", row);
		url = "../CarAction!updateCar";
	}
//ajax方式提交form表单	
	function saveCar() {
		$.ajax({
			...
			success : function(data) {
				...
				$("#dlg").dialog("close");
				$("#tt").datagrid({url:"../CarAction!carList"});							
				$("input[type=reset]").trigger("click");
			},
			error : function(data) {
...
<div id="body" region="center" > 
  ...
  <div id="search_area" >
    <div id="conditon">
      <table id="bb" border="0">
        <tr>
          <td>车辆编号：</td>
          <td ><input  name="carNo" id="carNo"/></td>
          <td>
              <a href="javascript:searchCar()" class="easyui-linkbutton" iconCls="icon-search" plain="true">查询</a> 
  ...
  <!-- 表格顶部工具按钮 -->
    <div id="tb">
        <a href="javascript:openCarAddDialog()" id="ddd" class="easyui-linkbutton" iconCls="icon-add" plain="true">新增</a> 
   ...
   <!-- 添加客户信息及修改客户信息 弹出框 -->
      ...
      <tr>
      <td>车辆编号：</td>
      <td><input type="text" name="carNo" id="carNo" class="easyui-validatebox" required="true" />
      &nbsp;<span style="color: red">*</span></td>
      </tr>
      <tr>
       <td>车辆ID号码：</td>
      <td><input type="text" name="vehicleId" id="vehicleId" class="easyui-validatebox" required="true" />
      &nbsp;<span style="color: red">*</span></td>
      </tr>
                </table>
                 <!--隐藏按钮,用于清空表单-->
