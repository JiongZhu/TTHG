...
<script>
	...
	function searchReception() {
		...
		$("#tt").datagrid('load', {
			...
			"reception.state":$("#state").combobox('getValue')
		});
	}
	//打开修改日志窗口
	...
	function deleteReception() {
		...
		for ( var i = 0; i < selectedRows.length; i++) {
			receptionIds.push(selectedRows[i].id);
		}
		var ids = receptionIds.join(",");
		$.messager.confirm("系统提示", "您确定要删除这 <font color=red>"
			...
			if (r) {
				$.ajax({
					type : "POST",
					dataType : "json",
					url : "../../ReceptionAction!deleteReception",
					data : "ids=" + ids,
					success : function(data) {
...
<body class="easyui-layout">
	<div id="body" data-options="region:'center'">
		...
		<!-- 表格顶部工具按钮 -->
		<div id="tb">
				<a href="javascript:openReceptionModifyDialog()"
				class="easyui-linkbutton"
				data-options="plain:true,iconCls:'icon-edit'">修改接待日志</a> <a
				href="javascript:deleteReception()" class="easyui-linkbutton"
				data-options="plain:true,iconCls:'icon-remove'">删除接待日志</a>
		</div>
		<!-- 添加接待信息及修改接待信息弹出框 -->
